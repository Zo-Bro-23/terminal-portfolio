...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const setCursorPos = (selectionPos) => {
            caretRef.current.style.left = `${USER_TEXT.length * CHAR_WIDTH - 1 + (selectionPos * CHAR_WIDTH) - (lineNumber * consoleInputDisplayRef.current.clientWidth)}px`
        }
        setCursorPos(0)
            ...
            if (!game) {
                switch (typedCommand.toLowerCase()) {
                    ...
                    case 'projects':
                        ...
                        const repositoriesJSON = await fetch(`https://api.github.com/users/${config.github}/repos?per_page=10&sort=updated&direction=desc`).then(res => res.json()).catch(() => {
